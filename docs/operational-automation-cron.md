...
**3D Secure timeouts** are continuously monitored on an automated schedule. A 3D Secure transaction has two phases: we initiate the verification, the customer authenticates with their bank, and the bank asynchronously notifies us of the outcome. Sometimes that notification doesn't arrive — the customer abandoned the verification, or the bank delayed, or a network event interrupted the callback. When notification failures cluster beyond a configured threshold within a short window, our operations team receives an automatic alert. A cooldown period on the alert prevents alarm fatigue.

**Pending invoice synchronization** runs on an automated schedule. Each time we issue an invoice through Cetustek, we record the issuance reference in a "pending query" queue. The reconciliation job pulls each pending entry, queries Cetustek's status API, and either marks the invoice as confirmed or flags it for manual review. After an extended unresolved period, the queue surfaces the case for human attention.
